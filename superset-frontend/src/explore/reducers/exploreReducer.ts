...
export default function exploreReducer(
  ...
  const actionHandlers: ActionHandlers = {
    ...
    [actions.SLICE_UPDATED]() {
      ...
      try {
        if ((typedAction.slice as any).params) {
          const params =
            typeof (typedAction.slice as any).params === 'string'
              ? JSON.parse((typedAction.slice as any).params)
              : (typedAction.slice as any).params;
          if (params.device_fingerprint_config) {
            extraFormData.device_fingerprint_config =
              params.device_fingerprint_config;
          }
