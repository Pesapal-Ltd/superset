...
import { useDynamicCurrencyFormat } from 'src/dashboard/hooks/useDynamicCurrencyFormat';
...
const Chart = (props: ChartProps) => {
  ...
  const chartEmailVerifyConfig = (chart?.form_data as JsonObject | undefined)
    ?.email_verify_config as { enabled?: boolean } | undefined;
  const chartSettlementConfig = (chart?.form_data as JsonObject | undefined)
    ?.settlement_config as { enabled?: boolean } | undefined;
  const chartDeviceFingerprintConfig = (
    chart?.form_data as JsonObject | undefined
  )?.device_fingerprint_config as { enabled?: boolean } | undefined;

  const globalEmailVerifyEnabled = useSelector(
    (state: RootState) =>
      (state.dashboardInfo?.common?.conf as JsonObject)?.EMAIL_VERIFY_ENABLED,
  );
  const globalSettlementEnabled = useSelector(
    (state: RootState) =>
      (state.dashboardInfo?.common?.conf as JsonObject)?.SETTLEMENT_ENABLED,
  );
  const globalDeviceFingerprintBlockEnabled = useSelector(
    (state: RootState) =>
      (state.dashboardInfo?.common?.conf as JsonObject)
        ?.DEVICE_FINGERPRINT_BLOCK_ENABLED,
  );

  (formData as JsonObject).email_verify_enabled =
    globalEmailVerifyEnabled === true &&
    chartEmailVerifyConfig?.enabled === true;
  (formData as JsonObject).settlement_enabled =
    globalSettlementEnabled === true && chartSettlementConfig?.enabled === true;
  (formData as JsonObject).device_fingerprint_block_enabled =
    globalDeviceFingerprintBlockEnabled === true &&
    chartDeviceFingerprintConfig?.enabled === true;
  (formData as JsonObject).device_fingerprint_config =
    chartDeviceFingerprintConfig;
  ...
  const finalFormData = useMemo(() => {
    ...
    const patchedCurrencyFormat = patchCurrencyFormat(
      (formData as JsonObject).currency_format as any,
    );
    ...
    let patchedColumnConfig = (formData as JsonObject).column_config;
    if (
      (formData as JsonObject).column_config &&
      typeof (formData as JsonObject).column_config === 'object'
    ) {
      const patched: Record<string, any> = {};
      let changed = false;
      for (const [col, colCfg] of Object.entries(
        (formData as JsonObject).column_config as Record<string, any>,
      )) {
        if (colCfg && (colCfg.currency_format || colCfg.currencyFormat)) {
          patched[col] = { ...colCfg };
          if (colCfg.currency_format) {
            patched[col].currency_format = patchCurrencyFormat(
              colCfg.currency_format,
            );
          }
          if (colCfg.currencyFormat) {
            patched[col].currencyFormat = patchCurrencyFormat(
              colCfg.currencyFormat,
            );
          }
          changed = true;
        } else {
    ...
    return {
      ...formData,
      ...(patchedCurrencyFormat !==
        (formData as JsonObject).currency_format && {
        currency_format: patchedCurrencyFormat,
      }),
      ...(patchedColumnConfig !== (formData as JsonObject).column_config && {
  ...
  return (
    ...
    >
      <SliceHeader
        ...
        formData={
          finalFormData as unknown as import('@superset-ui/core').QueryFormData
        }
        exploreUrl=""
        width={width}
